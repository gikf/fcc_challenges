function euler102() {
  ...
  /*
   * s = 1/(2*Area)*(p0y*p2x - p0x*p2y + (p2y - p0y)*px
   * + (p0x - p2x)*py); --> px, py coordinates of P
   * t = 1/(2*Area)*(p0x*p1y - p0y*p1x + (p0y - p1y)*px
   * + (p1x - p0x)*py);
  ...
  for (let i = 0; i < triangles.length; i++) {
    ...
    const [A, B, C] = [curTriangle[0], curTriangle[1], curTriangle[2]];
    let curArea = 0.5 * (-B[1]*C[0] + A[1] * (-B[0] + C[0]) + A[0] * (B[1] - C[1]) + B[0]*C[1]);
    let s = (1/(2*curArea)) * (A[1]*C[0] - A[0]*C[1]);
    let t = (1/(2*curArea)) * (A[0]*B[1] - A[1]*B[0]);
  ...
  console.log(trianglesInside);
  return trianglesInside;
}

const triangles = [[[-340, 495], [-153, -910], [835, -947]],
